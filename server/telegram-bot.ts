...
const formatQuestion = (question: TelegramQuestion): string => {
...
const formatResults = async (question: TelegramQuestion): Promise<string> => {
...
export async function broadcastQuestion(question: TelegramQuestion): Promise<number> {
  if (!bot) {
...
export async function broadcastResults(question: TelegramQuestion): Promise<number> {
  if (!bot) {
    console.error('Bot not initialized');
    return 0;
  }
